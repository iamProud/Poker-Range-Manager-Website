$(document).ready(function () {
    ...
    function watchColorPicker(event) {
        index = event.target.previousElementSibling.value;
        prevColor = colorsArray[index];
        newColor = event.target.value;
        handsArray.forEach(updateHandsArrayColors, prevColor, newColor);
        colorsArray[index] = newColor;
        handsArray.forEach(colorHand);
    }

    function updateHandsArrayColors(hand, handIndex) {
        if (hand != null && getHandIndex(hand, prevColor) != -1) {
            handsArray[handIndex][getHandIndex(hand, prevColor)].color = newColor;
        }
    }
    ...
    function getActionID_getRange_ajax(userID, rangeName, stacksize, rangeTitle) {
        $.ajax({
            ...
            error: function (e) {
                console.log(e);
                loadColorPicker();
                handsArray = new Array(numOfCards * numOfCards);
                handsArray.fill(null);
                handsArray.forEach(colorHand);
                handsArray = selectHand(handsArray);

                changeViewBackButton(handsArray);
            }
        });
    }
        ...
        if (handsArray[ii] == null) {
            ...
            if (handIndex != -1) {
                if (handsArray[ii][handIndex].frequency == frequency.value) {
                    ...
                    handsArray[ii] = checkFrequency(handsArray[ii], handsArray[ii][handIndex]);
                    cardsButtonHTMLcollection[ii].style.background = getColorString(handsArray[ii]);
                }
            } else {
                //different color
                ...
                handsUnsorted = checkFrequency(handsUnsorted, newHand);
    ...
    function colorHand(hand, handIndex){
        if (hand == null) {
            ...
            cardsButtonHTMLcollection[handIndex].style.background = getColorString(hand);
        }
    }
